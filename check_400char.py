#!/usr/bin/env python3
"""규정다름 — 서비스 동작 방식 설명 (400자 이내, 제출 양식 준수)"""
...
TEXT_400 = (
    ...
    "예외 처리: 구판/신판 중 하나라도 비어 있거나 조문 패턴(제N조)이 없으면 오류 메시지가 표시됩니다. "
    "로그인 없이 누구나 바로 사용할 수 있습니다. "
    "샘플: '샘플 불러오기'로 구판(2026.3.1. 시행)과 신판(2026.9.15. 시행) 텍스트를 바로 넣어볼 수 있습니다."
)

no_space = len(TEXT_400.replace(" ", "").replace("\n", ""))
print(f"공백 포함: {len(TEXT_400)} 자")
print(f"공백 제외: {no_space} 자")

if len(TEXT_400) <= 400:
    print(f"OK: {len(TEXT_400)}자 (400자 이내)")
elif no_space <= 400:
    print(f"OK: {no_space}자 (공백 제외 기준 400자 이내)")
else:
    print(f"OVER: {len(TEXT_400)}자")
